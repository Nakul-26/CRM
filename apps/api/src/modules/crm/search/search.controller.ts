...
export class SearchController {
  ...
  get(@CurrentUser() user: AuthenticatedUser, @Query("q") q: string, @Query("types") types?: string, @Query("limit") limit?: string) {
    ...
    const requested = types ? (types.split(",") as ("account" | "contact" | "lead")[]) : (["account", "contact"] as const);
    // A caller without crm.accounts.view / crm.contacts.view / leads.view is
    // silently excluded from that type rather than 403ing the whole search —
    // a Member who can see contacts but not accounts should still get
    ...
    const allowed = requested.filter(
      (type) =>
        ...
        (type === "lead" && user.permissions.includes("leads.view")),
    );
